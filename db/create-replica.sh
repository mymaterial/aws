...
if [ -z "$1" ]; then
    echo "Usage: ./bin/db/create-replica.sh  <<node name>>  [Instance type   db.t3.medium=default] [Engine version  15.3=default]"
    echo "       ./bin/db/create-replica.sh  node-02"
    exit
fi

SLEEP_TIME=10s
RDSA_CLUSTER_CF_STACK_NAME="pg-rdsa"
RDSA_REPLICA_CF_STACK_NAME="pg-rdsa-$1"
RDSA_REPLICA_NODE_NAME="pg-rdsa-$1"

TEMPLATE_LOCATION=file:///home/postgres/cloudformation/postgres-cluster-replica.yml \

# CF Parameters
RDSA_DB_CLUSTER_ID="pg-rdsa-cluster"
DB_INSTANCE_TYPE="db.t3.medium"

echo "DB Replica Node Name=$RDSA_REPLICA_NODE_NAME"
if [ -z "$2" ]; then
...
ParameterKey=ReplicaNodeName,ParameterValue="$RDSA_REPLICA_NODE_NAME" 
